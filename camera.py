...
from hardware_acceleration import get_hardware_info, get_yolo_device_kwargs, load_yolo_pose_model
...
from conversation_logger import ConversationLogger, HandshakeLogger
...
from conversation_tracker import ConversationTracker, get_display_name
from brochure_tracker import BrochureTracker, BrochureLogger, calibrate_brochure_rois
...
yolo_models = {name: load_yolo_pose_model("yolo26x-pose.pt") for name in quadrant_names}
...
HANDSHAKE_LOG_CSV = BASE_DIR / "logs" / "handshakes.csv"
BROCHURE_LOG_CSV = BASE_DIR / "logs" / "brochure_pickups.csv"

conversation_logger = ConversationLogger(csv_path=CONVERSATION_LOG_CSV)
handshake_logger = HandshakeLogger(csv_path=HANDSHAKE_LOG_CSV)
brochure_logger = BrochureLogger(csv_path=BROCHURE_LOG_CSV)
...
# Interactive Brochure Box ROI Calibration per camera feed
camera_brochure_rois = calibrate_brochure_rois(quadrant_names, ndi_manager.get_camera_frame)
brochure_tracker = BrochureTracker(rois=camera_brochure_rois)
...
def process_camera(cam_name: str, frame: np.ndarray) -> np.ndarray:
    ...
    if frame_index % process_every_n_frames == 0:
            ...
            for box_index, box in enumerate(r.boxes):
                ...
                if track_id != -1 and has_keypoints:
                    try:
                        ...
                        gesture_tracker.update_keypoints((cam_name, track_id), kp_xy, kp_conf)

                        person_label = get_display_name(cam_name, identity_manager, track_id)
                        curr_t = time.time()
                        if brochure_tracker.check_wrist_in_roi(
                            cam_name=cam_name,
                            track_id=track_id,
                            keypoints_xy=kp_xy,
                            ...
                            brochure_logger.log_pickup(camera=cam_name, person_name=person_label, timestamp_raw=curr_t)
                            postgres_logger.log_brochure_pickup_async(camera=cam_name, person_name=person_label, timestamp_raw=curr_t)
        ...
        if stale_ids:
            ...
            for stale_id in stale_ids:
                ...
                brochure_tracker.forget_track(cam_name, stale_id)
    ...
    brochure_tracker.draw_overlays(cam_name, frame, current_time=time.time())
    ...
    try:
        while True:
            ...
            conversation_tracker.finalize_global_conversations(
                ...
                handshake_logger=handshake_logger,
